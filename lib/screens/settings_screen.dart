...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  final TextEditingController _nameController =
      TextEditingController(); // NOVO: Controller do nome
  ...
  Future<void> _saveName() async {
    ...
    try {
      await Provider.of<FinanceState>(
        context,
        listen: false,
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Erro ao salvar nome: $e')));
      }
    } finally {
  ...
  Future<void> _unpairUser(String myUid) async {
    ...
    try {
      final myDocSnap = await FirebaseFirestore.instance
          .collection('partnerships')
          .doc(myUid)
          .get();
      ...
      if (myDocSnap.exists) {
        ...
        final batch = FirebaseFirestore.instance.batch();
        batch.delete(
          FirebaseFirestore.instance.collection('partnerships').doc(myUid),
        );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Erro ao desconectar: $e')));
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  }
                          ...
                          trailing: Row(
                            ...
                            children: [
                              IconButton(
                                ...
                                onPressed: () =>
                                    _showMyQrCode(context, user.uid),
                              ),
                              IconButton(
                  ...
                  _buildSectionCard(
                    ...
                    children: [
                      const Padding(
                        ...
                        child: Text(
                          "Faça login para salvar seus dados na nuvem e compartilhar com a família.",
                        ),
                      ),
                      Padding(
                        padding: const EdgeInsets.symmetric(
